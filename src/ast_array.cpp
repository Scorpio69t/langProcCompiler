...
ArrayDeclarator::ArrayDeclarator(DeclarationRoot* _declarator, ExpressionRoot* _array_size)
...
void ArrayDeclarator::EmitRISC(std::ostream& os, Context& context, int destReg) const { }

void ArrayDeclarator::Print(std::ostream& stream) const {
    stream << "ArrayDeclarator Operation";
}



ArrayIndex::ArrayIndex(ExpressionRoot* _array_id, ExpressionRoot* _index)
...
const ExpressionRoot* ArrayIndex::getIndex() const {
    return index;
}

void ArrayIndex::EmitRISC(std::ostream& os, Context& context, int destReg) const {
    // Get variable info
    ...
    index->EmitRISC(os, context, index_reg);
...
void ArrayInitialiser::EmitRISC(std::ostream& os, Context& context, int destReg) const {
    ...
    auto first_init = dynamic_cast<const ExpressionRoot*>(initialiser_list->front());
    ...
    for (auto init : *initialiser_list) {
        ...
        init->EmitRISC(os, context, init_reg);
...
void ArrayInitialiser::Print(std::ostream& stream) const {
    stream << "ArrayInitialiser Operation";
}
